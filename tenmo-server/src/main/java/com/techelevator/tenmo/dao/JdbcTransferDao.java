...
public class JdbcTransferDao implements TransferDao {
    ...
    public List<Transfer> displayPastTransfers(int userId) {
        ...
        sql = "SELECT transfer_id, username, amount " +
                "FROM tenmo_transfer " +
                "JOIN tenmo_account ON tenmo_transfer.account_to = tenmo_account.account_id " +
                "JOIN tenmo_user ON tenmo_account.user_id = tenmo_user.user_id " +
                "WHERE tenmo_user.user_id = ?";
        // STEP 3 - Send the SQL to the database and then store the results if necessary
        //         a) If we expect multiple rows or columns coming back (a spreadsheet) then we use jdbcTemplate.queryForRowSet
    ...
    public Transfer getTransferById(int transferId) {
        ...
        String sql = "SELECT transfer_id, from_user.username AS from_user, to_user.username AS to_user, transfer_type_desc, transfer_status_desc, amount " +
                "FROM tenmo_transfer " +
